...
struct VoteView: View {
        ...
        ZStack {
            VStack {
                viewModel.catImage
                    ...
                    .shadow(radius: 30.0)
                    .blur(radius: viewModel.shouldShowImage ? 0 : 30.0)
                    .padding()
                ScrollView {
                    VStack(spacing: 5){
                        HStack {
   ...
   /* func startFetchingCats() {
        // @State var isLoading = true (Actualiza el loader) -> @Published (en ObservedObject)
        // Cuando ya se hizo el fetch se indica que cambia el state de la vista
